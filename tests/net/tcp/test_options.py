...
class TestTcpOptions:
    ...
    def test_parse(self):
        opts = TcpOptions.parse(self._raw((
            ("NOP", ""),
            ("MSS", 1460),
            ("WScale", 8),
            ("SAckOK", ""),
    ...
    def test_quirks(self):
        ...
        assert opts == TcpOptions(
            ...
            quirks=(
                Quirk.OPT_EXWS | Quirk.OPT_ZERO_TS1
                | Quirk.OPT_NZ_TS2 | Quirk.OPT_EOL_NZ
            ),
            window_scale=15,
            eol_pad_length=2
        )
    ...
    def test_unexpected_option_length(self):
        assert TcpOptions.parse(bytes([TcpOption.MSS, 3, 1]), TcpFlag.SYN) == TcpOptions(
            layout=[TcpOption.MSS],
            quirks=Quirk.OPT_BAD
        )
